...
#===============================HARMONIC PART===============================#
def eachkfreq(kidx,FC2kspace,Fcc):
    kk = FC2kspace[kidx]#*math.pi/len_a
...
def harmFreq_per_k(FC2kspace):
    ...
    FCinput = FC2new.copy()
    for kidx in range(len(FC2kspace)):
        eigval,eigvector,D0 = eachkfreq(kidx,FC2kspace,FCinput.copy())
        for i in range(P):
...
def a2b2(m,km,Fpkpqkqrkr):
    ...
    for kq in range(len(FC3klist)):
        ...
        for p in range(4,P):
            for q in range(P):
                for r in range(P):
                    if (checkkpp(negekq,r) or checkkpp(kq,q) or ((m in [0,1,2,3]) and negekm == 0)):
                        pass
                    else:
...
#=============================== Find Root ============================================#
def intervals(km,m):# it is for e2 omega's intervals
        ...
        for _p in range(P):
            for _q in range(P):
                if (checkkpp(kp,_p) or checkkpp(kq,_q)):
                    ...
                    eachterm = w_omgkpcm[kp][_p] + w_omgkpcm[kq][_q]
                    diffthe = abs(eachterm - w_omgkpcm[km][m])
                    if(diffthe < check):
                        check =diffthe 
                        markk = [kp,_p,kq,_q]
...
#get the band of overtones
def plotofOvertone(markk):

    _p = markk[1]
    _q = markk[3]
    output = np.zeros((21,40)) 
    #for km in range(21):
    for km in range(21):
        idx = 0
        for kp in range(len(FC3klist)):
            kq = kqidxFC3gen(km,kp)
            output[km][idx] = (w_omgkpcm[kp][_p] +w_omgkpcm[kq][_q])
            idx+=1
    np.save('theOvertones',output)

def diagramsVdep(m,km,v):
    return e2(m,km,v,Fk) + f2(m,km,v,Fk) + a2b2(m,km,Fk)

#def fn_v_square(m,km,v,w):
#    return w**2 + 2 * w *diagramsVdep(m,km,v) - v**2
##-------------------------------------------------------------------------
#def bisect_root_find(m,km,interv):
#    #constantdiag = constdiag(m,km)
#    w = w_omgkp[3*km][m]
#    roots = []
#    for i in range(1,len(interv)):
#        left = interv[i - 1]  + 1E-15
#        right = interv[i] - 1E-15
#        leftv = fn_v_square(m,km,left,w)
#        rightv = fn_v_square(m,km,right,w)
#        print(left,right)
...
#                mid = left/2 + right/2
#                midv = fn_v_square(m,km,mid,w)
#                if abs(midv.real**2 + midv.imag**2) < 1E-6:
#                    root = mid
#                    break
...
#    for _p in range(P):
#        for _q in range(P):
#            for kp in range(len(FC3klist)):#kp runs over first BZ
#                #XXX: should be Fm-kmpkpq-(kp-km) but I took conjugate to Fmkmp-kpq(kp-km)
#                kq = kqidxFC3gen(kp,km)
#                if(km != 0):
#                    km2 = len(FC3klist) - km
#                else:
#                    km2 = km
#                if ((_p== 0 or _p == 1 or _p == 2 or _p == 3) and kp == 0) or ((_q== 0 or _q == 1 or _q == 2 or _q == 3) and kq == 0)  :
#                    pass
#                else:
#                    onetermF = Fk[km,kp,kq,m,_p,_q]/math.sqrt(len(FC3klist)*2**3*w_omgkp[km][m]*w_omgkp[kp][_p]*w_omgkp[3*kq][_q])
#                    retF +=  (onetermF.real**2 + onetermF.imag**2) /((- v - w_omgkp[kp][_p] - w_omgkp[kq][_q])**2)
#                    onetermE = Fk[km2,kp,kq,m,_p,_q]/math.sqrt(len(FC3klist)*2**3*w_omgkp[3*km2][m]*w_omgkp[kp][_p]*w_omgkp[kq][_q])
#                    retE += (onetermE.real**2 + onetermE.imag**2)/(( v - w_omgkp[kp][_p] - w_omgkp[kq][_q])**2)
#    derivative = 0.5*(-retE+retF)
#    return derivative
#
#def transitions(m,km,roots):
#    #transition = []
#    for v in roots:
#        derivative = transfn(m,km,v)
#         
#        intenss = w_omgkp[km][m]/(v - w_omgkp[km][m]*derivative)
#        #transition.append()
#    return transition

#get more K pointes for singularities
def detailedKpoints(m):
    harmout = []
    Anharmout = []
    kout = []
    #print(FC2klist)
    copyFC2klist = FC2klist.copy()
    for km in range(4,20): 
        FC2kspace = copyFC2klist.copy()
        #modigy FC2klist
        mid = FC2kspace[km]
        moreK = np.linspace(mid - 0.025,mid + 0.025,100)
        for each in moreK:
            FC2kspace[km] = each
            FC2kspace[40 - km] = -each
            harmFreq_per_k(FC2kspace.copy())
            kout.append(each)
            harmout.append(w_omgkpcm[km][m])
            Anharmout.append(math.sqrt(w_omgkp[km][m]**2 +2*w_omgkp[km][m]*(diagramsVdep(m,km,w_omgkp[km][m]).real))*transCon)
    out = np.array([kout,harmout,Anharmout])
    np.save("AllkmMoreforSingular",out)
    #listofk = [0.69,0.695,0.6975,0.699,0.6992,0.6994,0.6995,0.69985,0.69995,0.69999,0.699995,0.7,0.7001,0.7003,0.7005,0.702,0.705,0.708,0.71]
    #km = 14
    #for each in listofk:
    #    FC2klist[14] = each
    #    FC2klist[26] = -each 
    #    harmFreq_per_k()
    #    harmout.append(w_omgkpcm[14][5])
    #    Anharmout.append((w_omgkp[km][m] + diagramsVdep(m,km,w_omgkp[km][m])).real*transCon)
    #out = np.array([listofk,harmout,Anharmout])
    #np.save("morekforsingular",out)
def ModifyYellowOne():
    temp = np.zeros(21) 
    for km in range(10):
        m = 2
        temp[km] = (w_omgkp[km][m] + diagramsVdep(m,km,w_omgkp[km][m])).real*transCon
    for km in range(10,21):
        m = 1
        temp[km] = (w_omgkp[km][m] + diagramsVdep(m,km,w_omgkp[km][m])).real*transCon
    print(temp)
...
#ret,markk = intervals(14,5)
#plotofOvertone(markk)
