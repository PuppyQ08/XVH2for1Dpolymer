...
def bisect_root_find(m,km,interv,comb):
    ...
    for i in range(1,len(interv)):
        left = interv[i - 1]  + 1E-12
        right = interv[i] - 1E-12
...
def DysonRootsIntesity():
    km = 10 
    m = 14
    intervs,comb = intervals(km,m,[300,-300])
    ...
    #RootOutput = [[[] for i in range(4)] for j in range(len(FC3klist)//2+1)] 
    #TransOutput = [[[] for i in range(4)] for j in range(len(FC3klist)//2+1)] 
    #TheCombOutput = [[[] for i in range(4)] for j in range(len(FC3klist)//2+1)] 
    #range = [[60,30],[70,40],[50,20],[55,25] ]
    #for km in range(21):
    #    for m in range(14,18):
    #        intervs,comb = intervals(km,m,range[m-14])
    #        roots,realcomb = bisect_root_find(m,km,intervs,comb)
    #        availroots,trans,TheComb = transitions(m,km,roots,realcomb)
    #        RootOutput[km][m-14] = availroots
    #        TransOutput[km][m-14] = trans
    #        TheCombOutput[km][m-14] = TheComb 
    #        print("======================= The One Pointe is ")
    #        print("km ",km,"m ",m,"womg ",w_omgkpcm[km][m])
    #        for i in range(len(availroots)):
    #            print(availroots[i], " ", trans[i],"  ",TheComb[i])
    #Tosave=np.array([w_omgkpcm[:,14:18],TransOutput,RootOutput,TheCombOutput]) 
    ##np.save('1200DysonRootIntens_withoutYellow',Tosave)
    #np.save('3000DysonRootIntens_all',Tosave)
