...
def checkkpp(kp,p):
    ...
    if (((p in [0,1,2,3]) and kp == 0 ) or (p == 2 and kp in list(range(1,10))+ list(range(32,41))) or (p == 1 and kp in list(range(10,32))) or (p == 0) ):
            ...
            for _q in range(P):
                if (checkkpp(kp,_p) or checkkpp(kq,_q)):
                    ...
                    if not (eachterm in ret):
                        if (eachterm*transCon > w_omgkpcm[km][m] - 200 and eachterm*transCon < w_omgkpcm[km][m] + 100):
                            ret.append(eachterm)
    ret.sort()
    return ret
    ...
    for i in range(1,len(interv)):
        ...
        if np.sign(leftv) == np.sign(rightv):
            ...
            while abs(right - left) > 1E-15:
                ...
                if abs(midv.real**2 + midv.imag**2) < 1E-15:
                    root = mid
                    break
...
def transitions(m,km,roots):
    ...
    return availroots,transit

def DysonRootsIntesity():
    RootOutput = [[[] for i in range(P)] for j in range(len(FC3klist)//2+1)] 
    TransOutput = [[[] for i in range(P)] for j in range(len(FC3klist)//2+1)] 
    km = 0
    for m in range(4,P):
        intervs = intervals(km,m)
        roots = bisect_root_find(m,km,intervs)
        availroots,trans = transitions(m,km,roots)
        RootOutput[km][m] = availroots
        TransOutput[km][m] = trans
    for km in range(1,21):
        for m in range(P):
            ...
            RootOutput[km][m] = availroots
            TransOutput[km][m] = trans
    Tosave=np.array([w_omgkpcm,TransOutput,RootOutput]) 
    np.save('AllDysonRootIntens',Tosave)

def FreqIndep():
    temp = np.zeros((2,21,18),dtype=np.complex_)
    temp[0] = w_omgkpcm[:21].copy()
    km = 0
    for m in range(4,P):
        insidesqrt = (w_omgkp[km][m]**2 + 2*w_omgkp[km][m] * (diagramsVdep(m,km,w_omgkp[km][m]) + a2b2(m,km,Fk)).real)
        if(insidesqrt > 0):
            temp[1][km][m] = (math.sqrt(insidesqrt))*transCon
            print("wo",w_omgkpcm[km][m], "anharm", temp[1][km][m])
        else:
            print(km, m)
            temp[1][km][m] = w_omgkpcm[km][m]
    ...
    #            print(km,m)
    #            temp[1][km][m] = w_omgkpcm[km][m]
    #np.save('AnharmPhononDis_allrealMethod_fixBlue',temp)

    #XXX deal with blue Yellow line
    #m = 0 
    #blueYell = np.zeros((2,21))
    #for km in range(1,21):
    #    insidesqrt = (w_omgkp[km][m]**2 + 2*w_omgkp[km][m] * (diagramsVdep(m,km,w_omgkp[km][m])+ a2b2(m,km,Fk)).real)
    #    if(insidesqrt > 0):
    #        blueYell[0,km] = (math.sqrt(insidesqrt))*transCon
    #for km in range(1,21):
    #    if (km < 10):
    #        m = 2
    #        insidesqrt = (w_omgkp[km][m]**2 + 2*w_omgkp[km][m] * (diagramsVdep(m,km,w_omgkp[km][m])+ a2b2(m,km,Fk)).real)
    #        if(insidesqrt > 0):
    #            blueYell[1,km] = (math.sqrt(insidesqrt))*transCon
    #        
    #    elif (km >= 10):
    #        m = 1
    #        insidesqrt = (w_omgkp[km][m]**2 + 2*w_omgkp[km][m] * (diagramsVdep(m,km,w_omgkp[km][m])+ a2b2(m,km,Fk)).real)
